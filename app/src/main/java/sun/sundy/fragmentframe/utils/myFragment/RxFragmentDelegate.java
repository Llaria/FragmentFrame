...
public class RxFragmentDelegate {
    private static final String TAG = "RxFragmentDelegate";
     ...
     * @param to          目标Fragment
     */
    void loadRootTransaction(FragmentManager fragmentManager, int containerId, RxSupportFragment to) {
        RxSupportFragment fragment = findFragmentByRoot(fragmentManager, to.getClass().getName());
        if (fragment != null && containerId == fragment.getContainerId()) {
            return;
        }
        bindContainerId(containerId, to);
        dispatchStartTransaction(fragmentManager, null, to, 0, RxSupportFragment.STANDARD, TYPE_ADD);
    }
     ...
     * @param to          目标Fragment
     */
    void replaceLoadRootTransaction(FragmentManager fragmentManager, int containerId, RxSupportFragment to, boolean addToBack) {
     ...
     * 加载多个根Fragment
     */
    void loadMultipleRootTransaction(FragmentManager fragmentManager, int containerId, int showPosition, RxSupportFragment... tos) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
        FragmentTransaction ft = fragmentManager.beginTransaction();
        for (int i = 0; i < tos.length; i++) {
            RxSupportFragment to = tos[i];
     ...
     * @param type        类型
     */
    void dispatchStartTransaction(FragmentManager fragmentManager, RxSupportFragment from, RxSupportFragment to, int requestCode, int launchMode, int type) {
    ...
    private void bindContainerId(int containerId, RxSupportFragment to) {
     ...
     */
    void replaceTransaction(RxSupportFragment from, RxSupportFragment to, boolean addToBack) {
     ...
     * replace事务, 主要用于子Fragment之间的replace
     */
    void replaceTransaction(FragmentManager fragmentManager, int containerId, RxSupportFragment to, boolean addToBack) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
     ...
     */
    void showHideFragment(FragmentManager fragmentManager, RxSupportFragment showFragment, RxSupportFragment hideFragment) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
    ...
    void start(FragmentManager fragmentManager, RxSupportFragment from, RxSupportFragment to, String toFragmentTag, ArrayList<TransactionRecord.SharedElement> sharedElementList) {
        FragmentTransaction ft = fragmentManager.beginTransaction();
    ...
    void startWithPop(final FragmentManager fragmentManager, RxSupportFragment from, RxSupportFragment to, String toFragmentTag) {
        ...
        RxSupportFragment preFragment = getPreFragment(from);
    ...
    /**
     * 获得栈顶RxSupportFragment
     */
    RxSupportFragment getTopFragment(FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--) {
            ...
            if (fragment instanceof RxSupportFragment) {
                return (RxSupportFragment) fragment;
            }
        }
        return null;
    }
     ...
     */
    RxSupportFragment getPreFragment(Fragment fragment) {
        ...
        for (int i = index - 1; i >= 0; i--) {
            ...
            if (preFragment instanceof RxSupportFragment) {
                return (RxSupportFragment) preFragment;
            }
        }
        return null;
    }
    ...
    <T extends RxSupportFragment> T findStackFragment(Class<T> fragmentClass, String toFragmentTag, FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return null;
        ...
        if (toFragmentTag == null) {
            ...
            for (int i = sizeChildFrgList - 1; i >= 0; i--) {
                ...
                if (brotherFragment instanceof RxSupportFragment && brotherFragment.getClass().getName().equals(fragmentClass.getName())) {
     ...
     */
    private RxSupportFragment findFragmentByRoot(FragmentManager fragmentManager, String tag) {
        List<Fragment> fragmentList = fragmentManager.getFragments();
        if (fragmentList != null) {
            for (Fragment fragment : fragmentList) {
                if (fragment instanceof RxSupportFragment && tag.equals(fragment.getTag())) {
                    return (RxSupportFragment) fragment;
                }
            }
        }
     ...
     */
    RxSupportFragment getActiveFragment(RxSupportFragment parentFragment, FragmentManager fragmentManager) {
        List<Fragment> fragmentList = fragmentManager.getFragments();
        if (fragmentList == null) {
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--) {
            Fragment fragment = fragmentList.get(i);
            if (fragment instanceof RxSupportFragment) {
                RxSupportFragment supportFragment = (RxSupportFragment) fragment;
     ...
     */
    boolean dispatchBackPressedEvent(RxSupportFragment activeFragment) {
        if (activeFragment != null) {
            ...
            if (dispatchBackPressedEvent((RxSupportFragment) parentFragment)) {
                return true;
            }
        }
     ...
     * handle LaunchMode
     */
    private boolean handleLaunchMode(FragmentManager fragmentManager, RxSupportFragment toFragment, String toFragmentTag, int launchMode) {
        RxSupportFragment topFragment = getTopFragment(fragmentManager);
        ...
        if (launchMode == RxSupportFragment.SINGLETOP) {
            // 在栈顶
        ...
        } else if (launchMode == RxSupportFragment.SINGLETASK) {
    ...
    private void handleNewBundle(RxSupportFragment toFragment, Fragment stackToFragment) {
        ...
        ((RxSupportFragment) stackToFragment).onNewBundle(args);
    }
    ...
    private void debouncePop(RxSupportFragment from, FragmentManager fragmentManager) {
        // Debounce
        if (from == null) {
    ...
    void popTo(String fragmentTag, boolean includeSelf, Runnable afterPopTransactionRunnable, FragmentManager fragmentManager) {
        ...
        RxSupportFragment fromFragment = getTopFragment(fragmentManager);
     ...
     */
    private void handlePopAnim(Fragment targetFragment, RxSupportFragment fromFragment, RxSupportFragment toFragment) {
        ...
        RxSupportFragment preFragment = null;
